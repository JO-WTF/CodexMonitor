...
mod tests {
    ...
    fn request(raw: &str) -> HttpRequest {
        parse_request_headers(raw).expect("parse request")
    }

    #[test]
    fn parses_request_target_query_and_headers() {
        let request = request(
            "POST /api/rpc?token=dev%2Btoken HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nAuthorization: Bearer dev-token\r\n",
        );

        assert_eq!(request.method, "POST");
        assert_eq!(request.path, "/api/rpc");
        assert_eq!(request.query.as_deref(), Some("token=dev%2Btoken"));
        assert_eq!(
    ...
    fn rejects_invalid_request_line() {
        let error = parse_request_headers("GET\r\nHost: 127.0.0.1\r\n")
            .expect_err("invalid request should fail");

        assert_eq!(error, "invalid request line");
    }

    #[test]
    fn authorizes_bearer_token_before_query_token() {
        let request =
            request("GET /api/events?token=wrong HTTP/1.1\r\nAuthorization: Bearer expected\r\n");

        assert!(is_authorized(&request, Some("expected")));
        assert!(!is_authorized(&request, Some("wrong")));
    }

    #[test]
    fn authorizes_percent_decoded_query_token_for_eventsource() {
        let request = request("GET /api/events?token=dev%20token%2B1 HTTP/1.1\r\n");

        assert!(is_authorized(&request, Some("dev token+1")));
        assert!(!is_authorized(&request, Some("dev%20token%2B1")));
    }

    #[test]
    fn allows_requests_when_gateway_auth_is_disabled() {
        let request = request("GET /api/health HTTP/1.1\r\n");

        assert!(is_authorized(&request, None));
    }

    #[test]
    fn finds_header_terminator_and_status_reason() {
        assert_eq!(
        ...
        assert_eq!(reason_phrase(599), "OK");
    }
}
